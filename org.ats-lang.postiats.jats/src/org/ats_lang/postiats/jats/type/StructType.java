...
public class StructType extends ATSReferableType {
    ...
    class StructMemberLocation implements Location {
        ...
        public StructMemberLocation(Map<String, Object> content, String name,
                ATSReferableType elety) {
            m_content = content;
            m_name = name;
            m_elety = elety;
        }
    ...
    public Location getLoc(Object content, int offset, ATSReferableType loctype) {
        ...
        if (curtype.equals(loctype)) {
            // System.out.println("eeeeeeeeeeeeeeeeeeeeeeee 001 curtype is " +
            // curtype);
        ...
        if (curtype instanceof ATSEltType) {
            ...
            return ((StringType) curtype)
                    .getLoc(structsrc.get(curname), offset);
        } else if (curtype instanceof ArrayType) {
            return ((ArrayType) curtype).getLoc(structsrc.get(curname), offset,
                    loctype);
        } else if (curtype instanceof StructType) {
            return ((StructType) curtype).getLoc(structsrc.get(curname),
                    offset, loctype);
        } else {
            throw new Error("Type mismatch");
        }
