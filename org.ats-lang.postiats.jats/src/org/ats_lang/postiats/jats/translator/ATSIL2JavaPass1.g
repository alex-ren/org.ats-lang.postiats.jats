...
gstat
    : var_def[null] //    | var_assign {node = $var_assign.node;} no assignment for global variable
    ;
...
var_def [List<String> names]
    : ^(VAR atstype ID) {if (names != null) {names.add($ID.text);}} -> member_st(name={$ID.text})
    | ^(VAR_VOID atstype ID)
    ;
...
type_def
    : ^(TYPEDEF ID struct_def) -> class_st(cls_name={$ID.text}, cls_desc={$ID.line}, body={$struct_def.st})
    ;
...
List<String> names = new ArrayList<String>();
    ...
    : ^(STRUCT (vars+=var_def[names]
                )+
        ) -> body_st(members={vars}, names={names})
    ;
