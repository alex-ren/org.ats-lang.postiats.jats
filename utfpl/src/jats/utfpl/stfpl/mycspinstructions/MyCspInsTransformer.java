...
public class MyCspInsTransformer {
     ...
     */
    public ProgramMyCspIns trans(ProgramInstruction inputProg) {
        ...
        List<MyCspGroup> body = InsLst2CBlockLst2(inputProg.getInsLst(), subMap, mainLab);
        ...
        for (FunctionInstruction func : inputProg.getFuncLst()) {
            int pos = 0;
            FunctionMyCsp funcCSP = FunDef2CProcess(func, subMap);
        ...
        ProgramMyCspIns outputProg = new ProgramMyCspIns(globalVarInfo, body, procLst, inputProg.getExtCodeLst());
        return outputProg;
    }
...
//                    System.out.println("dddddddddddddddddd holder is " + ins.m_holder + "in " + m_funLab);
        ...
        public Object visit(InsCall ins) {
            if (ins.hasSideEffect()) {
                ...
                GrpProc cprocess = new GrpProc(ins.m_funlab, ins.m_isTailCall);
        ...
        public Object visit(InsStore ins) {
            ...
            IMyCspTemp localSrc = ValPrim2CTemp(ins.m_localSrc, m_subMap, m_funLab, m_cbEvt);
            MyCspTempID globalDest = TID2CTempID(ins.m_globalDest, m_subMap, m_funLab, m_cbEvt);
        ...
        public Object visit(InsThreadCreate ins) {
	        ...
	        IMyCspTemp args = ValPrim2CTemp(ins.m_args, m_subMap, m_funLab, nBlk);
        ...
        public Object visit(InsMCGet ins) {
            ...
            while (iter_gv.hasNext()) {
                ...
                MyCspTempID globalVar = TID2CTempID(iter_gv.next(), m_subMap, m_funLab, m_cbEvt);
                MyCspTempID localHolder = TID2CTempID(templv, m_subMap, m_funLab, m_cbEvt);
        ...
        public Object visit(InsMCSet ins) {
            ...
            while (iter_gv.hasNext()) {
                IMyCspTemp localSrc = ValPrim2CTemp(iter_lv.next(), m_subMap, m_funLab, m_cbEvt);
                MyCspTempID globalDest = TID2CTempID(iter_gv.next(), m_subMap, m_funLab, m_cbEvt);
    ...
    static private List<MyCspGroup> InsLst2CBlockLst2(List<UtfplInstruction> insLst, Map<TID, VariableInfo> subMap, TID funLab)
    {
     ...
     */
    static private IMyCspTemp ValPrim2CTemp(ValPrim vp, Map<TID, VariableInfo> map, TID funLab, MyCspGroup grp) {
        if (vp instanceof AtomValue) {
            return new MyCspTempVal((AtomValue) vp);
        } else if (vp instanceof TID) {
            TID tid = (TID)vp;
            return TID2CTempID(tid, map, funLab, grp);
        } else if (vp instanceof TupleValue) {
            return new MyCspTempVal((TupleValue) vp);
        } else {
     ...
     */
    static private List<IMyCspTemp> ValPrimLst2CTempLst(List<ValPrim> vpLst, Map<TID, VariableInfo> map, TID funLab, MyCspGroup grp) {
     ...
     */
    static private MyCspTempID TID2CTempID(TID tid, Map<TID, VariableInfo> map, TID funLab, MyCspGroup grp) {
...
//    }
    ...
    static private FunctionMyCsp FunDef2CProcess(
        ...
        for (TID para: funDef.getParaLst()) {
            MyCspTempID cPara = TID2CTempID(para, subMap, funDef.getName(), null /* CBlock is null */);
            paras.add(cPara);
        }
        ...
        List<MyCspGroup> body = InsLst2CBlockLst2(funDef.getBody(), subMap, funDef.getName());
        FunctionMyCsp cProc = new FunctionMyCsp(funDef.getName(), paras, body);
        return cProc;
    }
}
